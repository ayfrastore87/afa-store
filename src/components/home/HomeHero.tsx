...
import { isValidImageSource } from "@/lib/products";
...
export default function HomeHero({ products, onProducts }: { products: Product[]; onProducts: () => void }) {
  ...
  return <section id="beranda" className="relative overflow-hidden bg-[var(--cream)] text-[var(--dark-text)]" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
    <div className="mx-auto grid max-w-[1400px] items-center gap-5 px-5 py-7 sm:px-8 md:grid-cols-[42%_58%] md:py-8 lg:px-12">
      <AnimatePresence mode="wait" initial={false} custom={index}>
        <motion.div key={product?.id ?? "empty"} initial={{ opacity: 0, x: reduceMotion ? 0 : 26 }} animate={{ opacity: 1, x: 0, transition }} exit={{ opacity: 0, x: reduceMotion ? 0 : -26, transition: { duration: reduceMotion ? 0.1 : 0.45, ease: slideEase } }} className="relative z-10 max-w-xl">
          ...
          <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0, transition: { ...transition, delay: 0.1 } }} className="mt-3 min-h-[4.5rem] font-display text-4xl font-bold leading-[.95] tracking-tight sm:min-h-[5.7rem] sm:text-5xl lg:min-h-[6.8rem] lg:text-6xl">{product ? product.name : <>Pilihan Terbaik<br />Untuk Keluarga</>}</motion.h1>
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0, transition: { ...transition, delay: 0.17 } }} className="mt-4 min-h-[3rem] max-w-lg text-sm leading-relaxed text-[var(--muted)] sm:text-base">{product ? description : "Pilihan produk AFA STORE untuk keluarga, dipilih dari data produk yang dikelola Admin."}</motion.p>
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0, transition: { ...transition, delay: 0.24 } }} className="mt-7 grid grid-cols-2 gap-4 text-sm sm:grid-cols-4 md:grid-cols-2 lg:grid-cols-4"><MiniBenefit icon={<Leaf size={20} />} label="Produk Berkualitas" /><MiniBenefit icon={<Sparkles size={20} />} label="Rasa Istimewa" /><MiniBenefit icon={<ShieldCheck size={20} />} label="Aman & Higienis" /><MiniBenefit icon={<Heart size={20} />} label="Untuk Keluarga" /></motion.div>
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0, transition: { ...transition, delay: 0.3 } }}>{product ? <Link href={`/produk/${product.slug}`} className="group mt-6 inline-flex min-h-11 items-center gap-2 rounded-full bg-[#123524] px-6 py-2.5 text-sm font-bold text-white shadow-lg transition hover:bg-[#184D47]">Lihat Produk <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" /></Link> : <button type="button" onClick={onProducts} className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-full bg-[#123524] px-6 py-2.5 text-sm font-bold text-white shadow-lg transition hover:bg-[#184D47]">Lihat Produk <ArrowRight size={18} /></button>}</motion.div>
        </motion.div>
      </AnimatePresence>
