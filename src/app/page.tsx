...
import { ArrowUp, ChevronLeft, ChevronRight, Handshake, Headphones, Heart, MapPin, Search, Shield, ShoppingCart, Sparkles, Star, Trash2, Truck, User, X } from "lucide-react";
...
export default function Home() {
  ...
  const heroSlides = useMemo(() => banners.length ? banners : fallbackHeroProducts.map((product) => ({ id: product.id, title: product.name, subtitle: [product.flavor, product.size].filter(Boolean).join(" • ") || `Jelajahi ${product.category ?? "produk"} AFA STORE.`, image: product.image, ctaLabel: "Lihat Produk", ctaUrl: "/produk" })), [banners, fallbackHeroProducts]);
