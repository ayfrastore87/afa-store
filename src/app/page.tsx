...
export default function Home() {
  ...
  return <main data-theme={dark ? "dark" : "light"} className={`min-h-screen overflow-hidden bg-[#F8F5EE] text-[#123524] ${dark ? "theme-dark" : ""}`}><motion.div className="fixed left-0 right-0 top-0 z-80 h-1 origin-left bg-[#C9A45B]" style={{ scaleX: scrollYProgress }} />
    ...
    <section ref={parcelRef} id="parcel" className="product-section mx-auto max-w-[1440px] px-5 pb-28 pt-20 md:pb-20"><SectionTitle title="Parcel" /><ProductGrid loading={productsLoading} error={productsError} products={parcelProducts} parcel isWishlisted={isWishlisted} toggleWishlist={toggleWishlist} addCart={addCart} buyNow={buyNow} setPreview={setPreview} setZoom={setZoom} onRetry={loadProducts} /></section><Promo /><TestimonialsSection /><FAQ /><Footer /><Floating onCart={() => setCartOpen(true)} /><CartDrawer open={cartOpen} cart={cart} subtotal={subtotal} totalItems={totalItems} checkoutPending={checkoutPending} checkoutError={checkoutError} itemState={itemState} onClose={() => setCartOpen(false)} onCheckout={continueToCheckout} increaseQty={increaseQty} decreaseQty={decreaseQty} removeFromCart={removeFromCart} clearCart={clearCart} />
  </main>;
}
...
function ProductGrid({ loading, error, products, isWishlisted, toggleWishlist, addCart, buyNow, setPreview, setZoom, onRetry, parcel = false }: { loading: boolean; error: string; products: Product[]; parcel?: boolean; isWishlisted: (id: string) => boolean; toggleWishlist: (item: Product) => void; addCart: (item: Product) => void; buyNow: (item: Product) => void; setPreview: (preview: { src: string; name: string }) => void; setZoom: (zoom: number) => void; onRetry: () => void }) {
  ...
  if (!products.length) return <div className="luxury-card rounded-3xl p-8 text-center md:p-12"><h3 className="font-display text-2xl font-bold">{parcel ? "Parcel AFA" : "Belum ada produk yang sesuai."}</h3><p className="mx-auto mt-2 max-w-lg text-[#8B6B3F]">{parcel ? "Segera hadir dengan pilihan hampers spesial untuk berbagai momen." : "Coba kata kunci atau kategori lain untuk menemukan produk favorit Anda."}</p><a href="#katalog" className="mt-5 inline-flex min-h-11 items-center rounded-full border border-[#123524]/20 px-6 py-3 font-bold">{parcel ? "Jelajahi Produk" : "Reset Filter"}</a></div>;
...
function SectionTitle({ title }: { title: string }) { return <div className="mb-6 flex items-center justify-between"><h2 className="font-display text-3xl font-bold uppercase text-[#2E2A26]">{title}</h2><a href="#katalog" className="flex items-center gap-1 text-sm text-[#8B6B3F] hover:text-[#C8A45D]">Lihat Semua <ChevronRight size={16} /></a></div>; }
