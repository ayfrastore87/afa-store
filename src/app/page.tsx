...
export default function Home() {
  ...
  const [parcelPackages, setParcelPackages] = useState<ParcelPackage[]>([]), [parcelLoading, setParcelLoading] = useState(true), [parcelError, setParcelError] = useState("");
  ...
  const visibleProducts = useMemo(() => products.filter((p) => (filter === "Semua" || p.category === filter) && p.name.toLowerCase().includes(query.toLowerCase())).sort((a, b) => sort === "low" ? a.price - b.price : sort === "high" ? b.price - a.price : b.rating - a.rating), [filter, products, query, sort]);
  const bawangProducts = visibleProducts.filter((p) => !p.category.toLowerCase().includes("parcel"));
  ...
  useEffect(() => {
    fetchParcelPackages().then(setParcelPackages).catch((error: Error) => setParcelError(error.message)).finally(() => setParcelLoading(false));
  }, []);
  ...
  return <main data-theme={dark ? "dark" : "light"} className={`min-h-screen overflow-hidden bg-[#F8F5EE] text-[#123524] ${dark ? "theme-dark" : ""}`}><motion.div className="fixed left-0 right-0 top-0 z-80 h-1 origin-left bg-[#C9A45B]" style={{ scaleX: scrollYProgress }} />
    ...
    <section ref={katalogRef} id="bawang-goreng" className="product-section mx-auto max-w-7xl px-5 py-20"><SectionTitle title="Produk Bawang Goreng" /><div className="mb-6 flex gap-3 overflow-x-auto pb-2"><select onChange={(e) => setFilter(e.target.value)} className="rounded-full border border-[#C9A45B]/25 bg-white px-4 py-2 text-sm text-[#123524] shadow-sm"><option>Semua</option><option>Original</option><option>Pedas</option><option>Daun Jeruk</option></select><select onChange={(e) => setSort(e.target.value)} className="rounded-full border border-[#C9A45B]/25 bg-white px-4 py-2 text-sm text-[#123524] shadow-sm"><option value="featured">Rating terbaik</option><option value="low">Harga termurah</option><option value="high">Harga tertinggi</option></select>{productSizes.map((s) => <span key={s} className="shrink-0 rounded-full bg-white px-4 py-2 text-sm text-[#8B6B3F] shadow-sm">{s}</span>)}</div><ProductGrid loading={productsLoading} error={productsError} products={bawangProducts} isWishlisted={isWishlisted} toggleWishlist={toggleWishlist} addCart={addCart} buyNow={buyNow} setPreview={setPreview} setZoom={setZoom} /></section>
    <section ref={parcelRef} id="parcel" className="product-section mx-auto max-w-7xl px-5 pb-28 pt-20 md:pb-20"><SectionTitle title="Parcel" /><ParcelGrid loading={parcelLoading} error={parcelError} parcels={visibleParcelPackages} onAddCart={(item) => addCart({ id: item.id, name: item.name, slug: item.slug, price: item.price, image: item.image })} onOrderNow={(item) => buyNow({ id: item.id, name: item.name, slug: item.slug, price: item.price, image: item.image })} /></section><Promo /><Testimonials /><FAQ /><Footer /><Floating onCart={() => setCartOpen(true)} /><CartDrawer open={cartOpen} cart={cart} subtotal={subtotal} totalItems={totalItems} onClose={() => setCartOpen(false)} onCheckout={checkoutWhatsApp} increaseQty={increaseQty} decreaseQty={decreaseQty} removeFromCart={removeFromCart} clearCart={clearCart} /><MobileMenu open={mobileMenuOpen} onClose={setMobileMenuOpen.bind(null, false)} onSelectCategory={selectHeroCategory} /><ImagePreview preview={preview} zoom={zoom} onZoomIn={() => setZoom((z) => Math.min(2.5, z + 0.25))} onZoomOut={() => setZoom((z) => Math.max(1, z - 0.25))} onClose={() => setPreview(null)} /></main >;
}
...
function ParcelGrid({ loading, error, parcels, onAddCart, onOrderNow }: { loading: boolean; error: string; parcels: ParcelPackage[]; onAddCart: (parcel: ParcelPackage) => void; onOrderNow: (parcel: ParcelPackage) => void }) {
  if (loading) return <div className="luxury-card rounded-3xl p-8 text-center font-bold text-[#8B6B3F]">Memuat parcel...</div>;
  if (error) return <div className="luxury-card rounded-3xl p-8 text-center font-bold text-red-600">Gagal memuat parcel: {error}</div>;
  if (!parcels.length) return <div className="luxury-card rounded-3xl p-8 text-center font-bold text-[#8B6B3F]">Belum ada paket parcel yang tersedia.</div>;
  return <div className="product-grid grid auto-rows-fr grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">{parcels.map((parcel) => <ParcelCard key={parcel.id} item={parcel} onAddCart={() => onAddCart(parcel)} onOrderNow={() => onOrderNow(parcel)} />)}</div>;
}

function ParcelCard({ item, onAddCart, onOrderNow }: { item: ParcelPackage; onAddCart: () => void; onOrderNow: () => void }) {
  return <motion.article initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -6, scale: 1.01 }} className="luxury-card flex h-full min-w-0 flex-col overflow-hidden rounded-[24px] text-[#2E2A26] transition"><div className="p-3 pb-0 md:p-4 md:pb-0"><div className="relative aspect-square w-full overflow-hidden rounded-[24px] bg-linear-to-br from-[#FFF8EA] via-white to-[#EFE6D5]"><Image src={item.image} alt={item.name} fill sizes="(min-width: 1024px) 25vw, (min-width: 768px) 33vw, 50vw" className="object-contain p-3" />{item.badge && <b className="absolute left-3 top-3 rounded-full bg-[#C8A45D] px-3 py-1 text-xs text-white">{item.badge}</b>}</div></div><div className="product-card-info flex flex-1 flex-col px-3 pt-4 md:px-4"><h3 className="product-card-title min-h-[3.25rem] font-display text-lg font-bold leading-tight md:min-h-[3.5rem] md:text-xl">{item.name}</h3><p className="text-lg font-bold md:text-xl">{formatRupiah(item.price)}</p><p className="mt-2 text-sm text-[#8B6B3F]">{item.description}</p><p className="mt-2 text-xs font-bold uppercase tracking-wide text-[#8B6B3F]">{item.category}</p><div className="mt-3 flex flex-wrap gap-2">{item.contents.length ? item.contents.map((content, index) => <span key={`${item.id}-content-${index}`} className="rounded-full bg-[#FFF8EA] px-3 py-1 text-xs font-semibold text-[#8B6B3F]">{content}</span>) : <span className="rounded-full bg-[#FFF8EA] px-3 py-1 text-xs font-semibold text-[#8B6B3F]">Isi parcel menyesuaikan paket</span>}</div><p className="mt-2 text-sm font-semibold text-[#8B6B3F]">Status: {item.isActive ? "Aktif" : "Nonaktif"}</p></div><div className="product-card-actions mt-auto flex w-full min-w-0 items-center gap-3 p-4"><motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.98 }} onClick={onAddCart} aria-label={`Tambah ${item.name} ke keranjang`} className="product-card-cart-button grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#C8A45D] text-white shadow-[0_12px_24px_rgba(200,164,93,0.22)] hover:bg-[#A9853F]"><ShoppingCart size={20} /></motion.button><motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.98 }} onClick={onOrderNow} className="product-card-buy-button flex h-12 min-w-0 flex-1 items-center justify-center rounded-2xl border border-[#C8A45D] px-3 text-center text-base font-semibold leading-tight text-[#8B6B3F] hover:bg-[#C8A45D]/10">Beli Sekarang</motion.button></div></motion.article>;
}
...
type CartDrawerProps = { open: boolean; cart: CartItem[]; subtotal: number; totalItems: number; onClose: () => void; onCheckout: (shipping: number, discount: number) => void; increaseQty: (id: string) => void; decreaseQty: (id: string) => void; removeFromCart: (id: string) => void; clearCart: () => void };
