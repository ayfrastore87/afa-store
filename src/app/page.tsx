...
import { ArrowUp, Check, ChevronLeft, ChevronRight, Handshake, Headphones, Heart, Minus, Moon, Plus, Search, Shield, ShoppingCart, Star, Trash2, Truck, User, X } from "lucide-react";
...
function ProductGrid({ loading, error, products, isWishlisted, toggleWishlist, addCart, addState, buyNow, setPreview, setZoom, onRetry, parcel = false }: { loading: boolean; error: string; products: Product[]; parcel?: boolean; isWishlisted: (id: string) => boolean; toggleWishlist: (item: Product) => void; addCart: (item: Product) => void; addState: Record<string, "adding" | "added">; buyNow: (item: Product) => void; setPreview: (preview: { src: string; name: string }) => void; setZoom: (zoom: number) => void; onRetry: () => void }) {
...
function Footer() { return <footer id="kontak" className="mt-12 bg-[#17251d] text-white"><div className="section-shell grid gap-10 py-14 md:grid-cols-[1.4fr_1fr_1fr]"><div><Image src="/AFA LOGO.svg" alt="AFA STORE" width={64} height={84} className="mb-4 h-16 w-auto object-contain" /><h2 className="font-display text-3xl font-bold text-[#E4C982]">AFA STORE</h2><p className="mt-3 max-w-sm text-white/65">Rasa premium untuk setiap hidangan.</p></div><nav aria-label="Navigasi footer"><h3 className="font-bold">Jelajahi</h3><div className="mt-4 grid gap-3 text-white/70"><a href="#katalog">Produk</a><a href="#parcel">Parcel</a><a href="#tentang-kami">Tentang</a><a href="#faq">Bantuan</a><a href="/mitra">Jadi Mitra</a></div></nav><div><h3 className="font-bold">AFA Store</h3><p className="mt-4 text-white/70">Cilegon, Banten</p></div></div><div className="flex items-center justify-center border-t border-white/10 py-5 text-center"><AdminButton /></div></footer>; }
