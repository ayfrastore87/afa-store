...
import { ArrowUp, Check, ChevronLeft, ChevronRight, Handshake, Headphones, Heart, Moon, Search, Shield, ShoppingCart, Star, Trash2, Truck, User, X } from "lucide-react";
...
export default function Home() {
  const [cartOpen, setCartOpen] = useState(false), [dark, setDark] = useState(false);
  ...
  const [banners, setBanners] = useState<Banner[]>([]), [bannerIndex, setBannerIndex] = useState(0), [heroPaused, setHeroPaused] = useState(false);
  ...
  const selectedBanner = heroSlides.length ? heroSlides[bannerIndex % heroSlides.length] : undefined;
  const activeHero = selectedBanner ? { badge: "AFA STORE", title: [selectedBanner.title, ""], description: selectedBanner.subtitle, image: selectedBanner.image || "/products/parcel.png", target: "katalog" as const } : heroContent.bawang;
  ...
  useEffect(() => {
    setBannerIndex((current) => heroSlides.length ? current % heroSlides.length : 0);
  }, [heroSlides.length]);
  ...
  return <main data-theme={dark ? "dark" : "light"} className={`min-h-screen overflow-hidden bg-[#F8F5EE] text-[#123524] ${dark ? "theme-dark" : ""}`}><motion.div className="fixed left-0 right-0 top-0 z-80 h-1 origin-left bg-[#C9A45B]" style={{ scaleX: scrollYProgress }} />
    ...
    <header className="sticky top-0 z-50 min-h-[80px] border-b border-[#C9A45B]/15 bg-[#F8F5EE]/85 text-[#123524] shadow-[0_8px_28px_rgba(18,53,36,0.08)] backdrop-blur-xl md:min-h-[104px]"><div className="mx-auto flex min-h-[80px] max-w-[1440px] items-center justify-between gap-3 px-4 min-[393px]:px-5 md:min-h-[104px]"><a href="#beranda" className="group flex shrink-0 items-center gap-3 font-display text-2xl font-bold md:gap-4"><Image src="/AFA LOGO.svg" alt="AFA STORE" width={96} height={144} className="h-16 w-12 shrink-0 object-contain transition-transform duration-300 group-hover:scale-105 md:h-24 md:w-16" priority sizes="(max-width: 767px) 48px, 64px" /><span className="hidden leading-none md:inline">AFA STORE</span></a><div className="hidden flex-1 items-center rounded-full border border-[#C9A45B]/20 bg-white/80 px-5 py-2.5 shadow-sm md:flex"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari bawang goreng, parcel, atau rasa favorit..." className="w-full bg-transparent text-sm outline-none placeholder:text-[#8B6B3F]/70" /><Search size={18} className="text-[#C9A45B]" /></div><div className="flex shrink-0 items-center gap-1.5"><button onClick={() => setDark((current) => !current)} aria-label="Mode Gelap" title="Mode Gelap" aria-pressed={dark} className="grid h-10 w-10 place-items-center text-[var(--dark-text)] transition hover:scale-110 hover:brightness-110 active:scale-95"><Moon size={24} aria-hidden="true" /></button><button type="button" onClick={() => void openAccount()} aria-label="Akun" className="group flex h-10 items-center gap-1.5 px-1"><User size={20} className="shrink-0 text-[var(--dark-text)] transition-colors group-hover:text-[#C9A45B]" />{accountInitials && <span className="truncate text-[13px] font-bold uppercase leading-none text-[var(--dark-text)] transition-colors group-hover:text-[#C9A45B]">{accountInitials}</span>}</button><Link href="/wishlist" aria-label="Wishlist" className="relative grid h-10 w-10 place-items-center rounded-full hover:bg-[#C9A45B]/10"><Heart size={20} />{wishlistCount > 0 && <span className="absolute -right-1 -top-1 rounded-full bg-red-500 px-1.5 text-[10px] font-bold text-white">{wishlistCount}</span>}</Link><button onClick={() => void openCart()} aria-label={cartItemCount > 0 ? `Keranjang, ${cartItemCount} item` : "Keranjang"} className="relative grid h-10 w-10 place-items-center rounded-full hover:bg-[#C9A45B]/10"><ShoppingCart size={20} />{cartItemCount > 0 && <span className="absolute -right-1 -top-1 rounded-full bg-[#C9A45B] px-1.5 text-[10px] font-bold text-white">{formatBadge(cartItemCount)}</span>}</button></div></div></header>
    <section id="beranda" aria-roledescription="carousel" aria-label="Pilihan unggulan AFA STORE" onMouseEnter={() => setHeroPaused(true)} onMouseLeave={() => setHeroPaused(false)} onFocusCapture={() => setHeroPaused(true)} onBlurCapture={() => setHeroPaused(false)} className="relative mt-0 overflow-hidden bg-[radial-gradient(circle_at_82%_18%,rgba(201,164,91,0.42),transparent_30%),radial-gradient(circle_at_12%_18%,rgba(255,255,255,0.92),transparent_34%),linear-gradient(135deg,#FFFDF8_0%,#F8F5EE_50%,#FBF7EC_100%)] pt-4 text-[#123524]"><HeroDecor spicy /><div className="absolute right-[-30px] top-[30px] h-[240px] w-[240px] rounded-full bg-[#C9A45B]/25 blur-[90px]" /><div className="absolute inset-0 opacity-[0.04] [background-image:linear-gradient(#123524_1px,transparent_1px),linear-gradient(90deg,#123524_1px,transparent_1px)] [background-size:22px_22px]" /><AnimatePresence mode="wait" custom={heroSlideDirection}><motion.div key={selectedBanner?.id ?? "fallback"} custom={heroSlideDirection} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }} className="relative z-10 mx-auto grid min-h-[560px] w-full max-w-[1440px] grid-cols-[62%_38%] items-start gap-2 px-5 pb-10 pt-4 md:min-h-[640px] md:grid-cols-2 md:items-center md:gap-6 lg:grid-cols-[1.1fr_0.9fr]"><div className="relative z-20 max-w-[240px] text-left md:max-w-md md:pt-12"><span className="inline-flex rounded-full border border-[#C9A45B]/30 bg-white/75 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#C9A45B] shadow-sm backdrop-blur md:px-4 md:py-2 md:text-xs">{activeHero.badge}</span><h1 className="mt-4 font-display uppercase leading-none tracking-[-1px]"><span className="block break-words text-[40px] font-bold min-[390px]:text-5xl md:text-7xl">{activeHero.title[0]}</span><span className="block text-[30px] font-bold leading-none md:text-5xl">{activeHero.title[1]}</span></h1><p className="mt-4 max-w-[220px] text-base leading-[1.75] text-[#43503f] md:max-w-sm md:text-lg">{activeHero.description}</p><div className="mt-6 grid w-full gap-4"><motion.a whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} href={selectedBanner?.ctaUrl || whatsappUrl() || "#katalog"} className="inline-flex min-h-[52px] w-full items-center justify-center rounded-full bg-[#C9A45B] px-5 py-3 text-center text-base font-bold leading-normal text-white shadow-[0_18px_38px_rgba(201,164,91,0.34)] transition-all hover:bg-[#A7833A] md:min-h-[58px] md:text-lg">{selectedBanner?.ctaLabel || "Pesan Sekarang"}</motion.a><motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => scrollToSection(heroTargetRef)} className="min-h-[52px] w-full rounded-full border border-[#C9A45B] bg-white/60 px-5 text-sm font-bold text-[#123524] shadow-[0_16px_34px_rgba(18,53,36,0.10)] backdrop-blur transition-all hover:bg-[#C9A45B]/10 md:min-h-[58px]">Lihat Katalog</motion.button></div>{heroSlides.length > 1 && <div className="mt-5 flex items-center gap-2" aria-label="Pilih slide hero"><button type="button" onClick={() => goToHeroSlide(bannerIndex - 1)} aria-label="Slide sebelumnya" className="grid h-11 w-11 place-items-center rounded-full border border-[#C9A45B]/30 bg-white/70"><ChevronLeft size={18} /></button>{heroSlides.map((slide, index) => <button key={slide.id} type="button" onClick={() => goToHeroSlide(index)} aria-label={`Slide ${index + 1}: ${slide.title}`} aria-current={bannerIndex === index ? "true" : undefined} className={`h-2.5 rounded-full transition-all ${bannerIndex === index ? "w-8 bg-[#C9A45B] shadow-[0_8px_18px_rgba(201,164,91,0.35)]" : "w-2.5 bg-[#123524]/20 hover:bg-[#C9A45B]/60"}`} />)}<button type="button" onClick={() => goToHeroSlide(bannerIndex + 1)} aria-label="Slide berikutnya" className="grid h-11 w-11 place-items-center rounded-full border border-[#C9A45B]/30 bg-white/70"><ChevronRight size={18} /></button></div>}</div><div className="relative z-20 mt-14 flex justify-center self-start md:mt-0 md:translate-y-[-10px] md:self-center lg:justify-end"><HeroShowcase src={activeHero.image} /></div></motion.div></AnimatePresence></section>
    <CategoryCluster groups={categoryGroups} loading={productsLoading} onSelect={selectCatalogCategory} />
    <section className="relative z-10 mx-auto grid max-w-6xl grid-cols-2 gap-3 px-5 pb-8 md:grid-cols-4"><Info icon={<Truck />} title="Pengiriman Cepat" desc="" /><Info icon={<Shield />} title="Aman" desc="" /><Info icon={<Star />} title="HAMPERS" desc="" /><Info icon={<Headphones />} title="Support" desc="" /></section>
    {categoryGroups.map((group) => { const key = group.name.trim().toLowerCase(); return <section key={group.name} ref={key === "parcel" ? parcelRef : undefined} id={`kategori-${slugId(group.name)}`} className="product-section mx-auto max-w-[1440px] scroll-mt-28 px-5 pb-12 pt-12 md:pb-16"><SectionTitle title={group.name.toUpperCase()} subtitle={`${group.items.length} produk aktif dalam kategori ${group.name}`} /><ProductGrid loading={false} error="" products={group.items} parcel={key === "parcel"} isWishlisted={isWishlisted} toggleWishlist={toggleWishlist} addCart={addCart} addState={addState} buyNow={buyNow} onRetry={loadProducts} /></section>; })}
    <section id="katalog" ref={katalogRef} className="product-section mx-auto max-w-[1440px] scroll-mt-28 px-5 pb-28 pt-14 md:pb-20"><SectionTitle title={filter === "Semua" ? "SEMUA PRODUK" : `PRODUK ${filter.toUpperCase()}`} /><div className="mb-6 flex gap-3 overflow-x-auto pb-2"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari produk..." className="min-w-56 rounded-full border border-[#C9A45B]/25 bg-white px-5 py-2.5 text-sm shadow-sm outline-none" /><select value={filter} onChange={(e) => setFilter(e.target.value)} className="rounded-full border border-[#C9A45B]/25 bg-white px-4 py-2 text-sm text-[#123524] shadow-sm"><option>Semua</option>{categoryGroups.map((group) => <option key={group.name}>{group.name}</option>)}</select><select value={sort} onChange={(e) => setSort(e.target.value)} className="rounded-full border border-[#C9A45B]/25 bg-white px-4 py-2 text-sm text-[#123524] shadow-sm"><option value="featured">Rating terbaik</option><option value="low">Harga termurah</option><option value="high">Harga tertinggi</option></select></div><ProductGrid loading={productsLoading} error={productsError} products={visibleProducts} isWishlisted={isWishlisted} toggleWishlist={toggleWishlist} addCart={addCart} addState={addState} buyNow={buyNow} onRetry={loadProducts} /></section>
    <Promo /><TestimonialsSection /><FAQ /><Footer /><Floating onCart={openCart} totalItems={totalItems} /><CartDrawer open={cartOpen} cart={cart} subtotal={subtotal} totalItems={totalItems} checkoutPending={checkoutPending} checkoutError={checkoutError} itemState={itemState} onClose={() => setCartOpen(false)} onCheckout={continueToCheckout} increaseQty={increaseQty} decreaseQty={decreaseQty} removeFromCart={removeFromCart} clearCart={clearCart} />
  </main>;
}
...
function CategoryCluster({ groups, loading, onSelect }: { groups: { name: string; items: Product[] }[]; loading: boolean; onSelect: (category: string) => void }) {
  ...
  return <section aria-labelledby="category-cluster-title" className="category-cluster section-shell py-14 md:py-20"><div className="mb-7"><p className="text-xs font-extrabold uppercase tracking-[0.24em] text-[#A7833A]">Menu Belanja</p><h2 id="category-cluster-title" className="mt-2 font-display text-3xl font-bold sm:text-4xl">Pilih Kebutuhan Anda</h2><p className="mt-2 text-[var(--muted)]">Temukan produk AFA STORE sesuai kebutuhan Anda.</p></div><div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3 lg:grid-cols-4">{loading ? Array.from({ length: 2 }, (_, index) => <div key={index} className="skeleton min-h-52 rounded-[28px]" />) : groups.map((group) => { const normalized = group.name.toLowerCase(); const description = normalized === "bawang goreng" ? "Gurih • Renyah" : normalized === "parcel" ? "Hadiah & bingkisan" : "Pilihan produk AFA"; return <motion.button key={group.name} type="button" onClick={() => onSelect(group.name)} whileHover={{ y: -6 }} whileTap={{ scale: .98 }} className="category-menu-card group relative min-h-52 overflow-hidden rounded-[28px] border border-[var(--border)] bg-[var(--card)] p-4 text-left shadow-[0_16px_45px_rgba(18,53,36,0.10)] transition sm:min-h-64 sm:p-6"><div className="absolute -right-8 -top-8 h-28 w-28 rounded-full bg-[#C9A45B]/15 transition group-hover:scale-125" /><div className="relative mb-5 grid h-16 w-16 place-items-center overflow-hidden rounded-2xl bg-[#F1E7D3] text-3xl sm:h-20 sm:w-20"><ProductImage src={group.items[0]?.image ?? null} alt="" sizes="80px" /></div><h3 className="relative break-words font-display text-lg font-black leading-tight sm:text-2xl">{group.name}</h3><p className="relative mt-2 text-sm text-[var(--muted)]">{description}</p><div className="relative mt-5 flex items-center justify-between gap-2 text-sm font-extrabold text-[#A7833A]"><span>Lihat Produk →</span><span className="rounded-full bg-[#123524] px-2.5 py-1 text-xs text-white">{group.items.length}</span></div></motion.button>; })}</div></section>;
}
...
function ProductGrid({ loading, error, products, isWishlisted, toggleWishlist, addCart, addState, buyNow, onRetry, parcel = false }: { loading: boolean; error: string; products: Product[]; parcel?: boolean; isWishlisted: (id: string) => boolean; toggleWishlist: (item: Product) => void; addCart: (item: Product) => void; addState: Record<string, "adding" | "added">; buyNow: (item: Product) => void; onRetry: () => void }) {
  ...
  return <div className="product-grid grid auto-rows-fr grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">{products.map((p) => <ProductCard key={p.id} item={p} wish={isWishlisted(p.id)} onWish={() => toggleWishlist(p)} onAdd={() => addCart(p)} addState={addState[p.id]} onBuy={() => buyNow(p)} />)}</div>;
}
...
function CartToastView({ toast, onDismiss }: { toast: CartToast | null; onDismiss: () => void }) { return <AnimatePresence>{toast && <motion.div initial={{ opacity: 0, y: 24, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 24, scale: 0.98 }} role="status" aria-live="polite" className="fixed inset-x-0 bottom-5 z-100 flex justify-center px-4 md:inset-x-auto md:bottom-6 md:right-6 md:justify-end"><div className="w-full max-w-sm rounded-2xl border border-[#C8A45D]/30 bg-white p-4 text-[#2E2A26] shadow-[0_18px_45px_rgba(46,42,38,0.18)] md:w-96"><div className="flex items-start gap-3"><span aria-hidden="true" className={`mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full font-bold text-white ${toast.variant === "success" ? "bg-[#2e7d32]" : "bg-[#c62828]"}`}>{toast.variant === "success" ? "✓" : "!"}</span><div className="min-w-0 flex-1"><p className="font-bold leading-tight">{toast.title}</p><p className="mt-1 break-words text-sm text-[#8B6B3F]">{toast.message}</p>{toast.variant === "success" && <Link href="/cart" onClick={onDismiss} className="mt-3 inline-flex min-h-9 items-center justify-center rounded-full bg-[#123524] px-4 py-1.5 text-sm font-bold text-white transition hover:bg-[#1c5138]">Lihat Keranjang</Link>}</div><button type="button" onClick={onDismiss} aria-label="Tutup notifikasi" className="text-[#8B6B3F] transition hover:text-[#2E2A26]"><X size={18} /></button></div></div></motion.div>}</AnimatePresence>; }
function ProductCard({ item, onAdd, onBuy, onWish, wish, addState }: { item: Product; onAdd: () => void; onBuy: () => void; onWish?: () => void; wish?: boolean; addState?: "adding" | "added" }) { const categoryClass = item.category === "Parcel" ? "bg-orange-100 text-orange-700" : item.category === "Bawang Goreng" ? "bg-emerald-100 text-emerald-700" : "bg-[#F8F5EE] text-[#8B6B3F]"; const outOfStock = item.stock <= 0; return <motion.article initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -6, scale: 1.01 }} className="luxury-card product-card flex h-full min-w-0 flex-col overflow-hidden rounded-[24px] text-[#2E2A26] transition"><div className="p-3 pb-0 md:p-4 md:pb-0"><div className="relative"><Link href={`/produk/${item.slug}`} aria-label={`Lihat detail ${item.name}`} className="group relative block aspect-square w-full cursor-pointer overflow-hidden rounded-[24px] border border-transparent bg-linear-to-br from-[#FFF8EA] via-white to-[#EFE6D5] text-left transition duration-300 ease-out hover:border-[#C8A45D]/40 hover:shadow-[0_14px_38px_rgba(18,53,36,0.14)] active:scale-[0.985]"><motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.25, ease: "easeOut" }} className="h-full w-full"><ProductImage src={item.image} alt={item.name} sizes="(min-width: 1024px) 25vw, (min-width: 768px) 33vw, 50vw" /></motion.div>{item.badge && <b className="pointer-events-none absolute left-3 top-3 rounded-full bg-[#C8A45D] px-3 py-1 text-xs text-white">{item.badge}</b>}</Link><motion.button whileHover={{ scale: 1.14 }} whileTap={{ scale: 0.86 }} onClick={onWish} aria-label={wish ? `Hapus ${item.name} dari wishlist` : `Tambah ${item.name} ke wishlist`} className="absolute right-3 top-3 grid cursor-pointer rounded-full bg-white p-2 shadow transition duration-200 hover:bg-[#FFF8EA] active:scale-90"><Heart fill={wish ? "#ef4444" : "none"} className={wish ? "text-red-500" : "text-[#C8A45D]"} /></motion.button></div></div><div className="product-card-info flex flex-1 flex-col px-3 pt-4 md:px-4"><h3 className="product-card-title min-h-[3.25rem] font-display text-lg font-bold leading-tight md:min-h-[3.5rem] md:text-xl"><Link href={`/produk/${item.slug}`} aria-label={`Lihat detail ${item.name}`} className="inline cursor-pointer rounded transition duration-200 hover:text-[#8B6B3F] hover:underline active:text-[#8B6B3F]">{item.name}</Link></h3>{item.size ? <span className="mt-1 inline-flex w-fit max-w-full items-center rounded-full bg-[#F8F5EE] px-2.5 py-0.5 text-xs font-bold text-[#8B6B3F]">{item.size}</span> : null}<p className="text-sm text-[#C8A45D]">★★★★★ <span className="text-[#8B6B3F]">{item.rating > 0 ? item.rating.toFixed(1) : "Baru"}</span></p><div className="mt-2 flex items-center justify-between gap-2"><p className="font-bold text-[#8B6B3F]">{formatRupiah(item.price)}</p><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${categoryClass}`}>{item.category}</span></div><div className="mt-auto grid gap-2 pt-4"><button type="button" onClick={onAdd} disabled={outOfStock || addState === "adding" || addState === "added"} aria-busy={addState === "adding"} aria-label={outOfStock ? `${item.name} stok habis` : `Tambah ${item.name} ke keranjang`} className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-full border border-[#C8A45D]/40 font-bold text-[#8B6B3F] transition duration-200 hover:-translate-y-px hover:border-[#C8A45D]/70 hover:bg-[#FFF8EA] active:translate-y-0 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0 disabled:hover:border-[#C8A45D]/40 disabled:hover:bg-transparent">{addState === "adding" ? "Menambahkan..." : addState === "added" ? <><Check size={16} aria-hidden="true" /> Ditambahkan</> : outOfStock ? "Stok Habis" : "+ Keranjang"}</button><button type="button" disabled={outOfStock} onClick={onBuy} className="min-h-11 cursor-pointer rounded-full bg-[#123524] font-bold text-white transition duration-200 hover:-translate-y-px hover:bg-[#315d45] hover:shadow-[0_10px_26px_rgba(18,53,36,0.28)] active:translate-y-0 active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-[#9b9b91] disabled:hover:translate-y-0 disabled:hover:shadow-none">Beli Sekarang</button></div></div></motion.article>; }
...
function Footer() { return <footer id="kontak" className="mt-12 bg-[#17251d] text-white"><div className="section-shell grid gap-10 py-14 md:grid-cols-[1.4fr_1fr_1fr_1.2fr]"><div><Image src="/AFA LOGO.svg" alt="AFA STORE" width={64} height={84} className="mb-4 h-16 w-auto object-contain" /><h2 className="font-display text-3xl font-bold text-[#E4C982]">AFA STORE</h2><p className="mt-3 max-w-sm text-white/65">Rasa premium untuk setiap hidangan.</p></div><nav aria-label="Navigasi footer"><h3 className="font-bold">Jelajahi</h3><div className="mt-4 grid gap-3 text-white/70"><a href="#katalog">Produk</a><a href="#parcel">Parcel</a><a href="#tentang-kami">Tentang</a><a href="#faq">Bantuan</a></div></nav><div><h3 className="font-bold">AFA Store</h3><p className="mt-4 text-white/70">Cilegon, Banten</p></div><div className="rounded-2xl border border-[#C9A45B]/25 bg-gradient-to-br from-[#123524] to-[#1B4E42] p-5 shadow-[0_10px_30px_rgba(0,0,0,0.25)]"><h3 className="text-[11px] font-bold uppercase tracking-[0.22em] text-[#C9A45B]">Untuk Mitra</h3><p className="mt-3 text-sm leading-relaxed text-white/75">Bersama tumbuh lebih besar.<br />Jadi bagian dari AFA STORE.</p><Link href="/mitra" className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#C9A45B] px-5 py-3 text-sm font-bold text-[#123524] shadow-[0_8px_24px_rgba(201,164,91,0.35)] transition hover:scale-[1.03] hover:bg-[#E4C982] active:scale-95 sm:w-auto"><Handshake size={18} className="text-[#123524]" /> MITRA AFA_store</Link></div></div><div className="flex items-center justify-center border-t border-white/10 py-5 text-center"><AdminButton /></div></footer>; }
function Floating({ onCart, totalItems }: { onCart: () => void; totalItems: number }) { return <><button onClick={onCart} aria-label={totalItems > 0 ? `Buka keranjang, ${totalItems} item` : "Buka keranjang"} className="fixed bottom-[150px] right-[18px] z-40 grid h-14 w-14 place-items-center text-[var(--dark-text)] transition hover:scale-110 hover:brightness-110 active:scale-95"><span className="relative grid h-14 w-14 place-items-center"><ShoppingCart size={30} strokeWidth={2.2} className="drop-shadow-[0_3px_4px_rgba(18,53,36,0.35)]" />{totalItems > 0 && <span className="absolute -right-1 -top-1 rounded-full bg-[#C9A45B] px-1.5 text-[10px] font-bold text-white">{formatBadge(totalItems)}</span>}</span></button>{whatsappUrl("Halo AFA STORE, saya ingin bertanya tentang produk Anda.") && <a href={whatsappUrl("Halo AFA STORE, saya ingin bertanya tentang produk Anda.")!} target="_blank" rel="noopener noreferrer" aria-label="Chat WhatsApp" className="fixed bottom-[88px] right-[22px] z-40 grid h-12 w-12 place-items-center text-[#25D366] transition hover:scale-110 hover:brightness-110 active:scale-95"><svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className="h-7 w-7 drop-shadow-[0_3px_4px_rgba(37,211,102,0.40)]"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg></a>}<a href="#beranda" aria-label="Scroll ke atas" className="fixed bottom-5 right-[18px] z-40 grid h-14 w-14 place-items-center text-[#C9A45B] transition hover:scale-110 hover:brightness-110 active:scale-95"><ArrowUp size={30} strokeWidth={2.4} className="drop-shadow-[0_3px_4px_rgba(201,164,91,0.45)]" /></a></>; }
