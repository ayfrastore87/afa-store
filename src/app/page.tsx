...
function ParcelGrid({ loading, error, parcels, onOrderNow }: { loading: boolean; error: string; parcels: ParcelPackage[]; onOrderNow: (parcel: ParcelPackage) => void }) {
  ...
  return <div className="product-grid grid auto-rows-fr grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">{parcels.map((parcel) => <ParcelCard key={parcel.id} item={parcel} onOrderNow={() => onOrderNow(parcel)} />)}</div>;
}

function ParcelCard({ item, onOrderNow }: { item: ParcelPackage; onOrderNow: () => void }) {
  return <motion.article initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -6, scale: 1.01 }} className="luxury-card flex h-full min-w-0 flex-col overflow-hidden rounded-[24px] text-[#2E2A26] transition"><div className="p-3 pb-0 md:p-4 md:pb-0"><div className="relative aspect-square w-full overflow-hidden rounded-[24px] bg-linear-to-br from-[#FFF8EA] via-white to-[#EFE6D5]"><Image src={item.image} alt={item.name} fill sizes="(min-width: 1024px) 25vw, (min-width: 768px) 33vw, 50vw" className="object-contain p-3" />{item.badge && <b className="absolute left-3 top-3 rounded-full bg-[#C8A45D] px-3 py-1 text-xs text-white">{item.badge}</b>}</div></div><div className="product-card-info flex flex-1 flex-col px-3 pt-4 md:px-4"><h3 className="product-card-title min-h-[3.25rem] font-display text-lg font-bold leading-tight md:min-h-[3.5rem] md:text-xl">{item.name}</h3><p className="text-lg font-bold md:text-xl">{formatRupiah(item.price)}</p><p className="mt-2 text-sm text-[#8B6B3F]">{item.description}</p><p className="mt-2 text-xs font-bold uppercase tracking-wide text-[#8B6B3F]">{item.category}</p><div className="mt-3 flex flex-wrap gap-2">{item.contents.length ? item.contents.map((content) => <span key={content} className="rounded-full bg-[#FFF8EA] px-3 py-1 text-xs font-semibold text-[#8B6B3F]">{content}</span>) : <span className="rounded-full bg-[#FFF8EA] px-3 py-1 text-xs font-semibold text-[#8B6B3F]">Isi parcel menyesuaikan paket</span>}</div><p className="mt-2 text-sm font-semibold text-[#8B6B3F]">Status: {item.isActive ? "Aktif" : "Nonaktif"}</p></div><div className="mt-auto p-4"><motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.98 }} onClick={onOrderNow} className="h-12 w-full rounded-2xl border border-[#C8A45D] px-3 text-center text-base font-semibold leading-tight text-[#8B6B3F] hover:bg-[#C8A45D]/10">Pesan Sekarang</motion.button></div></motion.article>;
}
...
function HeroProduct({ src }: { src: string }) { return <motion.div animate={{ y: [0, -8, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }} className="relative z-20 aspect-square h-auto w-full overflow-visible border-none bg-transparent shadow-none"><Image src={src} alt="AFA Store produk premium" fill quality={100} sizes="(max-width: 767px) 220px, (max-width: 1023px) 340px, 460px" className="bg-transparent object-contain object-center drop-shadow-[0_28px_40px_rgba(18,53,36,0.18)]" priority /></motion.div>; }
